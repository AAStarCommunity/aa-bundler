...
func (q *userOpQueues) AddOp(entryPoint common.Address, op *userop.UserOp) {
...
func (q *userOpQueues) GetOps(entryPoint common.Address, entity common.Address) []*userop.UserOp {
	eps := q.getEntryPointSet(entryPoint)
	ess := eps.getEntitiesSortedSet(entity)
	nodes := ess.GetByRankRange(-1, -ess.GetCount(), false)
	batch := []*userop.UserOp{}
	for _, n := range nodes {
		batch = append(batch, n.Value.(*userop.UserOp))
	}

	return batch
}

func (q *userOpQueues) All(entryPoint common.Address) []*userop.UserOp {
	eps := q.getEntryPointSet(entryPoint)
...
func (q *userOpQueues) RemoveOps(entryPoint common.Address, ops ...*userop.UserOp) {
	eps := q.getEntryPointSet(entryPoint)
	for _, op := range ops {
		key := string(getUniqueKey(entryPoint, op.Sender, op.Nonce))
		eps.all.Remove(key)
